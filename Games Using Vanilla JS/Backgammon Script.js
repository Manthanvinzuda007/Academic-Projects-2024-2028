...
// State Variables
let state = {
    board: [], // Array of 24 objects { p: player, c: count }
    bar: { 1: 0, 2: 0 },
    off: { 1: 0, 2: 0 },
    turn: P1,
...
let checkersData = []; // { id, player, pIdx } mapping to DOM elements
let selectedChecker = null;
let validTargetsForSelected = [];
let allValidTurns = []; // Array of complete legal turns
let aiMoveQueue = []; // Queue for AI sequential moves
...
function rebuildCheckers() {
    ...
    // Using setTimeout to ensure DOM is updated before positioning
...
function createCheckerDom(id, player, pIdx) {
    ...
    // NO CLICK LISTENERS HERE ANYMORE - Handled by the Points directly

    checkersLayer.appendChild(el);
    checkersData.push({ id, player, pIdx, el });
}

// --- Point / Bar Click Handlers ---

pointsEl.forEach(pt => {
    pt.addEventListener('click', () => handlePointClick(parseInt(pt.dataset.idx)));
});
...
function handlePointClick(targetLoc) {
    ...
    if (modeSelect.value === 'ai' && state.turn === P2) return; // Prevent human interaction during AI turn

    // 1. Is it a bear-off click?
    ...
    // 2. Is it a valid move destination for currently selected checker?
    ...
    // 3. Try to select a checker at the clicked location
    let searchLoc = targetLoc;
    if (targetLoc === `bar${state.turn}`) searchLoc = 'bar'; // map UI id to logic id
    else if (targetLoc === 'bar1' || targetLoc === 'bar2') return; // clicked opponent's bar
    ...
    if (topId !== null) {
        ...
        // Only select our own
        if (cData.player !== state.turn) {
            clearSelection();
            return;
        }

        // Enforce "Must play from bar" rule
        ...
        // Toggle selection off if clicking same checker
        if (selectedChecker && selectedChecker.id === cData.id) {
            clearSelection();
            return;
        }

        // Select the new checker
        clearSelection();
        selectedChecker = cData;
        
        // Lift animation
        cData.el.classList.add('selected');
        // Bring to front based on selected
        ...
        // Clicked an empty point that isn't a valid move destination
        clearSelection();
    }
}
...
function cloneState(s) {
    return {
        ...
        turn: s.turn,
        dice: [...s.dice],
        originalDice: [...s.originalDice],
        diceRolled: s.diceRolled
    };
}

// Get raw valid destinations for a single move given a state and a specific die
function getValidMovesForDie(s, player, die) {
    let moves = [];
    let dir = (player === P1) ? -1 : 1;
    
    // Check if player is on bar
    if (s.bar[player] > 0) {
        let enterIdx = (player === P1) ? 24 - die : die - 1;
        if (canLandOn(s, enterIdx, player)) {
            moves.push({ from: 'bar', to: enterIdx, die: die });
        }
        return moves; // MUST move from bar first
    }
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
        if (s.board[i].p === player && s.board[i].c > 0) {
            ...
            if (target >= 0 && target < BOARD_SIZE) {
                if (canLandOn(s, target, player)) {
                    moves.push({ from: i, to: target, die: die });
                }
            } else if (canBearOff) {
                // Bearing off logic
                if (player === P1 && target === -1) {
                    moves.push({ from: i, to: 'off', die: die });
                } else if (player === P2 && target === 24) {
                    moves.push({ from: i, to: 'off', die: die });
                } else if ((player === P1 && target < -1) || (player === P2 && target > 24)) {
                    // Over-bearing. Valid if no pieces are further back.
                    let hasFurtherBack = false;
                    if (player === P1) {
                        for (let j = i + 1; j <= 5; j++) {
                            if (s.board[j].p === player && s.board[j].c > 0) { hasFurtherBack = true; break; }
                        }
                    } else {
...
function applyMoveToState(s, move, player) {
    ...
    if (move.from === 'bar') {
        ...
        ns.board[move.from].c--;
        if (ns.board[move.from].c === 0) ns.board[move.from].p = 0;
    }
...
// Generate all complete legal turns
function getLegalTurns(s, player, diceRemaining) {
        ...
        for (let m of moves) {
            ...
            if (nextDice.length > 0) {
                ...
                if (subTurns.length > 0) {
                    for (let st of subTurns) turns.push([m, ...st]);
                } else {
                    turns.push([m]);
                }
            } else {
    ...
    // Rule: Must play maximum possible dice
    let maxLen = Math.max(...turns.map(t => t.length));
    turns = turns.filter(t => t.length === maxLen);

    // Rule: If only 1 die can be played out of 2, must play the higher one if possible
    if (maxLen === 1 && s.originalDice.length === 2 && s.originalDice[0] !== s.originalDice[1]) {
        ...
        if (hasMaxDieMove) {
            turns = turns.filter(t => t[0].die === maxDie);
        }
    }
...
function getTopCheckerId(loc) {
    // loc is index 0-23 or 'bar'
    let chks = [];
    if (loc === 'bar') {
        // Find which bar we are looking at based on current turn
        chks = checkersData.filter(c => c.pIdx === 'bar' && c.player === state.turn);
    } else {
        chks = checkersData.filter(c => c.pIdx === loc);
    }
...
function clearSelection() {
    if (selectedChecker) {
        selectedChecker.el.classList.remove('selected');
        selectedChecker.el.style.zIndex = '';
    }
...
function executeMove(moveData) {
    ...
    // Save state for undo (only for human player)
    ...
    // Update Checker Data for visual transition
    let cData;
    if (moveData.from === 'bar') {
        cData = checkersData.find(c => c.player === state.turn && c.pIdx === 'bar');
    } else {
    ...
    if (cData) {
        cData.pIdx = moveData.to;
        cData.el.style.zIndex = 500; // Bring to front while animating
        setTimeout(() => cData.el.style.zIndex = '', 400); // Reset after transition
    }
    ...
    if (state.dice.length > 0) {
        ...
        if (allValidTurns.length === 0) {
            showNotification("No more valid moves.");
            aiMoveQueue = [];
...
function rollDice() {
    ...
    setTimeout(() => {
        ...
        if (d1 === d2) {
            ...
            state.originalDice = [d1, d1, d1, d1];
            showNotification("Doubles!");
        } else {
            state.dice = [d1, d2];
            state.originalDice = [d1, d2];
        }
    ...
    }, 400); // 0.4s roll animation
}

btnRoll.addEventListener('click', rollDice);

function updateDiceUI() {
    let orig = [...state.originalDice];
    let curr = [...state.dice];
...
function switchTurn() {
    if(state.off[1] === 15 || state.off[2] === 15) return; // Game over
    ...
    // AI FIX: Safely call rollDice for the computer instead of clicking the disabled button
...
// --- Rendering & Exact Positioning Math ---

function updateUI() {
    turnDisplay.textContent = state.turn === P1 ? "Player 1 (White)'s Turn" : "Player 2 (Black)'s Turn";
...
function updatePlayableHighlights() {
    ...
    playableOrigins.forEach(origin => {
        let locStr = origin === 'bar' ? 'bar' : origin; // getTopCheckerId expects 'bar' and handles player internally
...
function renderCheckers() {
    // We use the layer boundaries to absolutely position accurately regardless of CSS borders outside
    const layerRect = checkersLayer.getBoundingClientRect();
    if(layerRect.width === 0) return; // Prevent calc if not rendered yet
    
    // Determine the exact rendered size of a checker
    ...
    const positionGroup = (grp, targetEl, isTop, isBarOrOff = false) => {
        ...
        // Accurate Base Coordinates based on the Layer
        ...
        // Triangles are 90% height, let's keep checkers perfectly within that
        if (!isBarOrOff) usableHeight = targetRect.height * 0.90; 

        // Overlap logic if checkers exceed the height of the point
        ...
        grp.forEach((c, idx) => {
            ...
            let y;
            if (isBarOrOff) {
                // Stack towards center vertically for bar
                ...
                if (isTop) {
                    ...
                    y = baseY + targetRect.height - checkerSize - (idx * stepY);
                }
            }
            c.el.style.transform = `translate(${x}px, ${y}px)`;
            
            // Add numerical badge if stacked deeply (> 5)
    ...
    // Position Points
    pointsEl.forEach(pt => {
        let idx = parseInt(pt.dataset.idx);
        let isTop = pt.classList.contains('top-row');
    ...
    // Borne off checkers are tracked via numbers, so hide them off-screen smoothly
...
function getPipCount(s, player) {
    let pips = 0;
    for (let i = 0; i < BOARD_SIZE; i++) {
        if (s.board[i].p === player) {
            pips += s.board[i].c * (player === P1 ? (i + 1) : (24 - i));
        }
    }
...
function evaluateBoard(s, player) {
    ...
    // Pips difference (positive score if we are ahead)
    score += (getPipCount(s, opp) - getPipCount(s, player)) * 2;
    
    // Borne off
    score += s.off[player] * 300;
    score -= s.off[opp] * 300;
    
    // Bar
    score -= s.bar[player] * 150;
    score += s.bar[opp] * 150;
    ...
    for (let i = 0; i < BOARD_SIZE; i++) {
        ...
        if (pt.p === player) {
            if (pt.c > 1) {
                score += 15; // Prime/Block
                if ((player === P1 && i <= 5) || (player === P2 && i >= 18)) score += 20; // Home prime
            } else if (pt.c === 1) {
                // Blot penalty (worse if opponent is on bar)
                score -= s.bar[opp] > 0 ? 30 : 15;
            }
        } else if (pt.p === opp) {
            if (pt.c === 1) score += 10; // Target opportunity
        }
    }
    return score;
}
...
function processAITurn() {
    ...
    // Execute from queue if already calculated
    ...
    // AI LOGIC - Merged to one strong AI
    ...
    for (let turn of allValidTurns) {
        ...
        for (let move of turn) {
            tempState = applyMoveToState(tempState, move, P2);
        }
        let score = evaluateBoard(tempState, P2);
        
        // AI penalizes blots heavily
    ...
    // Load queue and execute first move
...
function checkWinCondition() {
    if (state.off[1] === 15 || state.off[2] === 15) {
        let winner = state.off[1] === 15 ? "Player 1 (White)" : "Player 2 (Black)";
